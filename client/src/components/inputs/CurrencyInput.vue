...
export default defineComponent({
  ...
  methods: {
    ...
    getErrorMessage(text: string) {
      const randValue = text ? +text.substring(2).trim() : 0;
      if (randValue <= 0) {
        return "Please enter a number greater than zero."
      }

      const pattern = /^\d+(\.\d{1,2})?$/;
      if (!pattern.test(randValue.toString())) {
  ...
  watch: {
    input() {
      ...
      this.input = `R ${this.input.replace(/[^0-9.]/g, "").trim()}`;
      this.validateInput();

      this.$emit('input', this.input.substring(2).trim());
    },
